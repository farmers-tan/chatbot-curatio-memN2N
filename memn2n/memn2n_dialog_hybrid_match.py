...
class MemN2NDialogHybridMatch(object):
    ...
    def __init__(self, 
        ...
        self._build_inputs()
        print("MemN2N Hybrid with Glove and Match features")
...
#         self._answers = tf.placeholder(tf.int32, [None, self._vocab_size], name="answers")
    ...
    def _build_vars_adj(self):
        with tf.variable_scope(self._name):
            ...
            self._U_matrices.append(tf.Variable(U_emb, name='U'))
        ...
        self._nil_vars = set([mat.name for mat in self._weight_matrices + self._T_weight_matrices + self._U_matrices])
    ...
    def _inference_adj(self, stories, queries, temporal, linear_start, match_features=None):
        with tf.variable_scope(self._name):
            ...
            u = [u_0]
            u_emb = tf.nn.embedding_lookup(self._U_matrices[0], queries)
            ...
            if self._match:
                ...
                return tf.matmul(tf.matmul(u[-1], self.Wa), self._answer_n_hot)
...
#             self.W = tf.Variable(self._init([self._embedding_size, self._vocab_size]), name="W")
        ...
        self._nil_vars = set([self.A.name, self.B.name, self.C.name, self.TA.name, self.TC.name, self.U_emb])
